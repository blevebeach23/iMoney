...
export function FixedExpenseForm({
  ...
  if (categories.length === 0 || containers.length === 0) {
    return (
      <div className="space-y-3 rounded-md border border-border bg-white p-4">
        ...
        <p className="text-sm leading-6 text-zinc-600">Crea almeno un conto/fondo e una categoria prima di aggiungere spese fisse.</p>
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <input type="hidden" name="householdId" value={fixedExpense?.householdId ?? defaultHousehold?.id ?? ""} />
